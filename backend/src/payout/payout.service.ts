  ...
  async create(createPayoutDto: CreatePayoutDto) {
    try {
      ...
      if(!fName || !email || !phone || !method || !amount){
        throw new BadRequestException('fName,email,phone,method and amount are mandatory')
      }
      if(method === 'crypto'  ){
        if(!currency){
      ...
      await Payout.create({fName,email,phone,method,amount,account,currency,network,walletAddress})
