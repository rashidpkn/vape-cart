    ...
    <Card>
      <Table sx={{overflow:'scroll'}} >
        <TableHead>
          ...
          <TableBody>
            {deliverySettings.map(e=><Row  row={e} setDeliverySettings={setDeliverySettings}/>
          )}
          <TableRow>
...
<TableRow key={day}>    
              <TableCell>
              <Checkbox checked={enable} onChange={e => setDeliverySettings(prev => {
    return prev.map(item => {
        ...
        return item;
              ...
              <TableCell>
                <TextField disabled={!enable} sx={{width:'200px'}} type='time' size='small' label='Open Time' value={open} onChange={e => setDeliverySettings(prev => {
    return prev.map(item => {
        if (item.day === day) {
...
/>
              ...
              <TableCell>
              <TextField disabled={!enable} sx={{width:'200px'}} type='time' size='small' label='Close Time' value={close} onChange={e => setDeliverySettings(prev => {
    return prev.map(item => {
        if (item.day === day) {
...
/>
              ...
              <TableCell>
              <TextField disabled={!enable} sx={{width:'200px'}} type='time' size='small' label='Last Call' value={lastCall} onChange={e => setDeliverySettings(prev => {
    return prev.map(item => {
        if (item.day === day) {
